...
async def start(client, message):
    ...
    if len(message.command) != 2:
        buttons = [[
            ...
            InlineKeyboardButton('🚫 Jᴏɪɴ Uᴘᴅᴀᴛᴇ Cʜᴀɴɴᴇʟ 🚫', callback_data='channels')
        ]]
        ...
        async for msg in client.iter_messages(int(f_chat_id), int(l_msg_id), int(f_msg_id)):
            if msg.media:
                ...
                try:
                    ...
                    asyncio.create_task(delete_after_delay(k, 60))
                    asyncio.create_task(delete_after_delay(h, 60))
                except FloodWait as e:
                    await asyncio.sleep(e.x)
...
# Ask Doubt on telegram @KingVJ01
    ...
    if not files_:
        ...
        try:
            ...
            title = '@YD_OWNER_BOT  ' + ' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), file.file_name.split()))
